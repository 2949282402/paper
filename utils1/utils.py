...
def get_network(arch: str, isTrain=False, continue_train=False, init_gain=0.02, pretrained=True):
    # ============ FreqNet support (added for RGB branch replacement) ============
    if "freqnet" in arch:
        from networks.freqnet import FreqNet, freqnet

        if isTrain:
            if continue_train:
                model: FreqNet = freqnet(num_classes=1)
            else:
                # FreqNet does not have pretrained weights, initialize from scratch
                model: FreqNet = freqnet(num_classes=1)
                # Initialize fc layer
            ...
            model: FreqNet = freqnet(num_classes=1)
        return model
    # ============ End of FreqNet support ============

    # ============ Original ResNet support ============
    elif "resnet" in arch:
    # if "resnet" in arch:  # Original code (commented out)
        ...
        if isTrain:
            ...
            model: ResNet = resnet(num_classes=1)
        return model
    # ============ End of Original ResNet support ============
