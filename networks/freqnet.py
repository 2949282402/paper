"""
FreqNet: Frequency-Aware Deepfake Detection Network
From AAAI 2024 paper: "Frequency-Aware Deepfake Detection: Improving Generalizability through Frequency Space Domain Learning"
Source: https://github.com/chuangchuangtan/FreqNet-DeepfakeDetection

Modified for AIGVDet project - removed hardcoded .cuda() calls for device flexibility
"""
...
class FreqNet(nn.Module):
    """
    FreqNet: Frequency-aware network for deepfake detection.

    Key components:
    - HFRI (High-Frequency Region Isolation): 2D FFT with center masking
    - HFRFC (High-Frequency Response on Channel): 1D FFT along channel dimension
    - FCL (Frequency Convolution Layer): Complex-valued convolution in frequency domain
    - ResNet layers for spatial feature learning
    """

    def __init__(self, block=Bottleneck, layers=[3, 4], num_classes=1, zero_init_residual=False,
                 feature_dim=256, return_feature=False):
        super(FreqNet, self).__init__()
        self.return_feature = return_feature

        # Frequency domain learnable parameters
        # Modified: removed .cuda() for device flexibility
        ...
        # Feature projection layer: 512 -> feature_dim
        self.feature_fc = nn.Linear(512, feature_dim)
        # Classification layer (only used when return_feature=False)
    ...
    def hfreqWH(self, x, scale):
        """
        HFRI: High-Frequency Region Isolation in spatial dimensions (Width & Height)
        - Apply 2D FFT
    ...
    def hfreqC(self, x, scale):
        """
        HFRFC: High-Frequency Response on Channel dimension
        - Apply 1D FFT along channel dimension
        - Zero out low-frequency center region
        - Apply inverse FFT
        """
        assert scale > 2
    ...
    def forward(self, x, *args):
        ...
        Added *args for compatibility with AIGVDet's meta parameter passing.
        """
        ### HFRI
        x = self.hfreqWH(x, 4)
...
def freqnet(pretrained=False, **kwargs):
    """
    Constructs a FreqNet model.
    Note: pretrained weights are not available for FreqNet, parameter kept for API consistency.
    """
    model = FreqNet(**kwargs)
    return model
